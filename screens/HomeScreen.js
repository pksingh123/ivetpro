...
export default class HomeScreen extends Component {
  ...
  async createNotificationListeners() {
    ...
    this.notificationOpenedListener = firebase
      ...
      .onNotificationOpened((notificationOpen) => {
        ...
        if (notificationOpen.notification._data.type === "calling") {
          this.showAlert(
            notificationOpen.notification._data.title,
            notificationOpen.notification._data.body,
            notificationOpen.notification
          );
        } else if (notificationOpen.notification.data.type === "chat") {
    ...
    if (
      notificationOpen.notification._data.type === "calling" &&
  ...
  showAlert(title, body, notification) {
    ...
    this.props.navigation.navigate("IncominCall", {
      data: notification,
      dialername: notification._data.dialer,
      onPick: (data) => {
  ...
  componentDidMount() {
    ...
    getAppstoreAppMetadata("com.videowithmyvet") //put any apps packageId here
      .then((metadata) => {
        ...
        let aLast = appStoreVersion.substring(
          lastdotx + 1,
          appStoreVersion.length
        );
  ...
  async setHomeScreenData() {
    ...
    this.appointment_booking =
      savedValues.user.practice.appointment_bookingin_app_allowed;
    let prevent_calling =
      savedValues.user.practice.prevent_phone_app_calling_agent;
    ...
    if (this.state.uid) {
      ...
      const url =
        Constant.rootUrl +
        "webservices/client-pet.php?action=alive&uid=" +
        this.state.uid;
      fetch(url)
  ...
  getButtonText() {
    return this.state.showDeceasedPets
      ? "Hide Deceased Pet"
      : "Show Deceased Pet";
  }
  getRightText() {
  ...
  handleNavigateToChat = (item) => {
    ...
    this.props.navigation.navigate("ChatScreen", {
      id: item.id,
      bookingId: item.nextAppointment.bookingId,
    });
  };
          ...
          <View style={styles.slideInnerContainer}>
            ...
            >
              ...
              >
                ...
                >
                  {index > 0 ? (
                    <TouchableOpacity
                      onPress={() => this._leftArrowClick(index)}
                    >
                      <Image
                ...
                >
                  {index < this.state.GridViewItems.length - 1 ? (
                    <TouchableOpacity
                      onPress={() => this._rightArrowClick(index)}
                    >
                      <Image
            ...
            <View style={styles.container}>
              ...
              <View style={styles.blockStyle}>
                ...
                {item.isUpcomingApp ? (
                  <View style={styles.itemsContentStyle}>
                    <Text style={styles.itemHeadingStyle}>
                      Video consultation appointment.
                    </Text>
                    <Text style={styles.itemStyle}>
                      {item.nextAppointment.date} at {item.nextAppointment.time}
                    </Text>
                    <Text style={styles.itemStyle}>
                      {this.state.practice_name}
                    </Text>
                    {item.nextAppointment.paid == 1 &&
                    this.state.prevent_phone_app_calling_agent == 0 ? (
                      <View style={styles.buttoncontainer}>
                        <TouchableOpacity
                          onPress={() => this._videoConsultation(item)}
                          style={styles.button}
                        >
                          <Text
                            style={
              ...
              {item.status == 1 && this.appointment_booking == 1 ? (
                <View style={styles.buttoncontainer}>
                  <TouchableOpacity
                    onPress={() => this._bookAppointment(item)}
                    style={styles.button}
                  >
                    <Text
                      style={
              ...
              <View style={styles.blockStyle}>
                ...
                >
                  ...
                  {item.pastAppointment && item.pastAppointment.length ? (
                    <TouchableOpacity
                      onPress={() => this._PastAppointments(item)}
                    >
